function [mean_dw, mean_dphi, dw, dphi, mean_dw_part, dw_part] = DORK_frequency(filename,idx_k0,Ni)
...
ref_idx = 51;% Steady-state time frame to use as reference
...
%     data_TN(:,i:Ni:end) = data_TN(:,1:Ni:end) - repmat(data_TN(:,ref_idx(1)),1,size(data_TN(:,1:Ni:end),2));
%     data_TE(:,i:Ni:end) = data_TE(:,1:Ni:end) - repmat(data_TE(:,ref_idx(1)),1,size(data_TE(:,1:Ni:end),2));
%     data_TN(:,i:Ni:end) = data_TN(:,i:Ni:end) - repmat(data_TN(:,ref_idx(1)),1,size(data_TN(:,i:Ni:end),2));
%     data_TE(:,i:Ni:end) = data_TE(:,i:Ni:end) - repmat(data_TE(:,ref_idx(1)),1,size(data_TE(:,i:Ni:end),2));
end
