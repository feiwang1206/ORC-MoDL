...
%                 x = nufft_adj_simple(A.segment_filter{m} .* (tmp(A.segment_index{m})), A.scaling_factor, A.interpolation_matrix{m}, A.oversampling, A.imageDim);
                x = gather(nufft_adj_simple_gpu(A.segment_filter{m} .* (tmp(A.segment_index{m})), A.scaling_factor, A.interpolation_matrix{m}, A.oversampling, A.imageDim));
                if ndims(A.wmap) == 3
                    y = y + conj(A.wmap(:,:,m)) .* x;
...
%                 x = nufft_simple(x, A.scaling_factor, A.interpolation_matrix{m}, A.oversampling);
                x = gather(nufft_simple_gpu(x, A.scaling_factor, A.interpolation_matrix{m}, A.oversampling));
