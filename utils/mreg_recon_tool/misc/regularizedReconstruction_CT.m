function [z, z_iter] = regularizedReconstruction_CT(A,AT,b,image_size,varargin)
...
%         function [y1, re] = f(z,alpha,d,update_flag) % nargin>=2 is meant for efficient line search
%             if nargin==1
%                 [y1, re] = LS(z);
%                 for m=1:length(R)
%                     y1 = y1 + R{m}(z);
...
%         end
            ...
            if nargin==1
                [y1, re] = LS(z);
                for m=1:length(R)
                    [yR,reR{m}]=R{m}(gather(z));
                    y1 = y1 + yR;
                end
            else
                [y1, re] = LS(z,alpha,d,update_flag);
                for m=1:length(R)
                    [yR,reR{m}]=R{m}(gather(z),alpha,gather(d),update_flag);
                    y1 = y1 + yR;
                end
            end
        end
