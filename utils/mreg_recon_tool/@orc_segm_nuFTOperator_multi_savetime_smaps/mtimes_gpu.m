    ...
    if A.adjoint
        ...
        for m=1:length(A.interp_filter)
            ...
            if length(A.imageDim)==3
                x = gather(prod(A.imageDim)*ifft(ifft(ifft(gpuArray(x),[],3),[],2),[],1));
                if any(A.imageDim < A.oversampling)
                    x = x(1:A.imageDim(1),1:A.imageDim(2),1:A.imageDim(3),:);
                end
            else
                x = gather(prod(A.imageDim)*ifft(ifft(gpuArray(x),[],2),[],1));
                if any(A.imageDim < A.oversampling)
...
%         Q = sum(Q,4)/sqrt(prod(A.imageDim))/length(A.tADC);
        ...
        if length(A.imageDim)==3
            ...
            for m=1:length(A.interp_filter)
                ...
                x = gather(reshape(fft(fft(fft(gpuArray(x),A.oversampling(3),3),A.oversampling(2),2),A.oversampling(1),1),[],A.numCoils));
            ...
            for m=1:length(A.interp_filter)
                ...
                x = gather(reshape(fft(fft(gpuArray(x),A.oversampling(2),2),A.oversampling(1),1),[],A.numCoils));
                Q = Q + A.interp_filter{m}*x;
            end
        end
