...
%             tralength=prod(A.imageDim);
%             tem=reshape(B((k-1)*tralength+1:k*tralength),A.imageDim);
%             Q = Q + prod(A.imageDim)*ifftn(tem) .* conj(A.sensmaps{k});
        end
        % Normalization
...
%             Q((k-1)*A.trajectory_length+1:k*A.trajectory_length) = nufft((B.*A.sensmaps{k}), A.nufftStruct);
            Q(:,k) = nufft((B.*A.sensmaps{k}), A.nufftStruct);
%             tralength=prod(A.imageDim);
%             tem=(fftn(B.*A.sensmaps{k}));
%             Q((k-1)*tralength+1:k*tralength) = tem(:);
        end
        Q = Q(:) / sqrt(prod(A.imageDim));
