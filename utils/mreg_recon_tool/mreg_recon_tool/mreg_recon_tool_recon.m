...
if gpuDeviceCount==0
    ...
    acc=1;
        ...
        if recon_details.offresonance_correction_flag==1
            if acc==0
%                 A = orc_segm_nuFTOperator(K,dim,data.smaps,data.wmap,dwelltime,10,recon_details.DeltaT);
                A = orc_segm_nuFTOperator_multi_sub({K},dim,(data.smaps),(data.wmap),dwelltime,10,...
                    {recon_details.DeltaT+(1:length(K))*5e-6});
%                 A = org_seg_gpuNUFFT(K'/pi/2,1,5,2,dim,data.smaps,data.wmap,0,dwelltime,recon_details.DeltaT,true,false,false);
            else
                A = orc_segm_nuFTOperator_multi_sub({K},dim,gpuArray(data.smaps),gpuArray(data.wmap),dwelltime,10,...
                    {recon_details.DeltaT+(1:length(K))*5e-6});
            end
            ...
            if acc==0
                A = nuFTOperator(K,dim,data.smaps);
%                 A = gpuNUFFT(K'/pi/2,ones(size(K)),1,5,2,dim,data.smaps);
            else
                A = nuFTOperator(K,dim,gpuArray(data.smaps));
            end                
        end
        ...
        if strcmp(recon_details.recon_type,'sliding window') || strcmp(recon_details.recon_type,'KWIC')
            ...
            if current_tframes(1)>=1 && current_tframes(end)<=Nt
                ...
                recon_tf = regularizedReconstruction(A,gpuArray(b(:)),P{:}, ...
                    'tol',recon_details.tolerance, ...
                    'maxit',recon_details.max_iterations, ...
                    'verbose_flag', verbose_flag, ...
                    'cg_method', recon_details.cg_method, ...
            ...
            if acc==0
                ...
                recon_tf = regularizedReconstruction(A,gpuArray(rawdata),P{:}, ...
                    'tol',recon_details.tolerance, ...
                    'maxit',recon_details.max_iterations, ...
                    'verbose_flag', 0, ...
                    'cg_method', recon_details.cg_method, ...
                    'z0', gpuArray(recon_details.z0));
                recon_tf=gather(recon_tf);
