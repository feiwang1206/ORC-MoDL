...
% Fei Wang
    ...
    rawdata_header = readHeader(recon_details.rawdata_filename);
...
if acc==1
    smaps=gpuArray(data.smaps);
    wmap=gpuArray(data.wmap);
else
    smaps=data.smaps;
    wmap=data.wmap;
end
...
%         Fg=orc_segm_nuFTOperator_multi_sub(K,size(wmap),smaps,wmap,dt,10,T);
    else
        Fg=nuFTOperator_multi(K,size(wmap),smaps,'','','minmax:kb');
    end
    ...
    for k=1:length(recon_details.penalty)
        ...
        counter = counter + 1;
        P{counter} = 5e-6;%recon_details.penalty(k).lambda;
        counter = counter + 1;
