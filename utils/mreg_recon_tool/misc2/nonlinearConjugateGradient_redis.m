function [z, z_iter] = nonlinearConjugateGradient_redis(f,df,z,maxit,cg_method,verbose_flag,reference_norm)
...
%     end
    ...
    gnorm = l2norm(dfz);
    z_iter{5}(iter)=alpha*gnorm/reference_norm;
    z_iter{3}(iter)=alpha*gnorm;
    if gpuDeviceCount>0
        fz=gather(fz);
    end
    frec(iter) = fz;
    ...
    if verbose_flag==1
        ...
        axis off;
        title(sprintf('iter %d: step = %1.5e, f(z) = %1.5e, step2 = %1.5e\n', iter, alpha*gnorm/reference_norm, fz, alpha*gnorm));
        drawnow;
