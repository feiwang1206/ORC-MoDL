function [out, hdr] = loadData_local(filename,t)
...
twx_obj_name = [filename_no_ext, '_twx_obj_local.mat'];

if ~exist([pathname,'/twx_obj_local'],'dir')
    mkdir([pathname,'/twx_obj_local']);
end

if ~exist(fullfile([pathname,'/twx_obj_local'],twx_obj_name),'file')
    twx = mapVBVD(filename);
    if iscell(twx)
        twx = twx{end};
    end
    save(fullfile([pathname,'/twx_obj_local'],twx_obj_name),'twx');
else
    load(fullfile([pathname,'/twx_obj_local'],twx_obj_name));
end
...
elseif length(t)>1
    ...
    %following is not very elegant. Couldn't fins all parameters in the header though
...
%             end
        ...
        if(cast(version,'double')>67) %if sofware version D or later
            ...
            hdr.trajectIndices = reshape(cell2mat(twx.hdr.MeasYaps.sWipMemBlock.alFree(33:56)),[hdr.trajectorySegments 2]);
        else
            hdr.trajectory = twx.hdr.MeasYaps.sWiPMemBlock.tFree(2:end-1);
