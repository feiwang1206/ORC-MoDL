function T = stack_of_spirals_axis_equalz_mshot_diff(R,Nradial,Nz,fov,resolution, gap,b,xyz)
...
Td(1).G=zeros(1500,3);
if maxg~=0
    Td(1).G(1:500,xyz)=0:maxg/500:(maxg-maxg/500);
    Td(1).G(501:1000,xyz)=maxg;
    Td(1).G(1001:1500,xyz)=(maxg-maxg/500):-maxg/500:0;
end
Td(1).G(1501:3000,:)=-Td(1).G;
Td(1).K=gamma*cumsum(Td(1).G)*1e-5;

T0=T;
for ii=1:length(T0)
    T(ii)=T0(ii);
    T(ii).K=[b*Td(1).K;T0(ii).K];
    T(ii).G=[b*Td(1).G;T0(ii).G];
end
