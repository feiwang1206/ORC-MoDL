function T = stack_of_spirals_axis_unequalz_mshot(R,Nradial,Nz,fov,resolution, ang,gap)
...
kz=kz(1:min(30,length(kz)));
kr_max=kr_max(1:min(30,length(kr_max)));
...
%         end
%         T(element, iz, 1) = trajectStruct_rotate(T(element, iz, 1),ang*2*pi,[0 0 1]);
        T(element, iz, 1) = trajectStruct_rotate(T(element, iz, 1),(element/length(kz)+ang)*2*pi,[0 0 1]);
