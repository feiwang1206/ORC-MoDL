function T = stack_of_spirals_axis2_mshot(R,Nradial,Nz,fov,resolution, pf)
...
for k=1:length(T0)
    for ii=1:8
        T(k).K(ii*length(T0(k).K)+(1:length(T0(k).K)),:)=T0(k).K;
        T(k).G(ii*length(T0(k).K)+(1:length(T0(k).G)),:)=T0(k).G;
    end
    T(k+length(T0))=T(k);
    T(k+length(T0)).K=-T(k).K;
    T(k+length(T0)).G=-T(k).G;
end
