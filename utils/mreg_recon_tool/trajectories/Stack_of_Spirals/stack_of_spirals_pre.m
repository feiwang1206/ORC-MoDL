function T = stack_of_spirals_pre(R,Nradial,Nz,fov,resolution,traj)
...
gamma=4258*1e4;
T(1).K=-traj.trajectory{1}(1:2:end,:)/pi*k_max;
T(1).G=([T(1).K;[0 0 0]]-[[0 0 0];T(1).K])/1e-5/gamma;
T(1).G=T(1).G(1:end-1,:);
T(1).index=[floor(traj.idx{1}(1)/2) floor(traj.idx{1}(end)/2)];

T(2)=T(1);
T(2).K=-T(1).K;
T(2).G=([T(2).K;[0 0 0]]-[[0 0 0];T(2).K])/1e-5/gamma;
T(2).G=T(2).G(1:end-1,:);
