function T = stack_of_spirals_anisofov(R,Nz,fov,resolution, sphericity, pf)
...
while kz(i) < k_max-kz_shift*(Nz-1)/2
    kz(i+1) = min(kz(i) + (Rz_min + kz(i)/k_max * (Rz_max - Rz_min))/fov(2), k_max);
    kr_max(i+1) = max(sqrt(kr_max(1)^2 - sphericity*kz(i+1)^2), k_max/10);
    i = i + 1;
end
...
for iz=2:Nz
    T(iz) = T(1);
    T(iz) = trajectStruct_rotate(T(iz),(2*pi/Nz)*(iz-1),[0 0 1]);
end
T(2)=T(1);
