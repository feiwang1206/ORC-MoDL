...
if 0 % check approximation accuracy
	max_percent_diff 'Ge_zmap * xtrue(mask)' 'Gm * xtrue(mask)'
return
end
...
if ~isvar('xcg1'), printm 'xcg1 iterative'
	...
	xinit = xcp0;
	cpu tic
	tic;xcg1 = qpwls_pcg1(xinit(mask), Gm, 1, yi(:), R.C, 'niter', f.niter);toc
...
% 	if 0 & ~isvar('xcg2'), printm 'xcg2'
	if ~isvar('xcg2'), printm 'xcg2'
		tic;xcg2 = qpwls_pcg2(xinit(mask), Tm, bb, R.C, 'niter', f.niter);toc
